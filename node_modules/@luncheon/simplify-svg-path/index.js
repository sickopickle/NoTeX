...
const fit = (points, tdata, closed, error) => {
    ...
    fitCubic(points, tdata, segments, error, 0, length - 1, 
    // Left Tangent
...
const fitCubic = (points, tdata, segments, error, first, last, tan1, tan2) => {
...
const simplifySvgPath = (points, tdata, options = {}) => {
    ...
    return getSegmentsPathData(fit(points.map(typeof points[0].x === 'number' ? (p) => new Point(p.x, p.y) : (p) => new Point(p[0], p[1])), tdata, options.closed, options.tolerance ?? 2.5), options.closed, options.precision ?? 5);
};
