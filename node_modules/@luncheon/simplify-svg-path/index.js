...
const fit = (points, tdata, closed, error) => {
    ...
    const gammas = fitCubic(points, tdata, segments, error, 0, length - 1, 
    // Left Tangent
...
const fitCubic = (points, tdata, segments, error, first, last, tan1, tan2, Gammas) => {
    ...
    for (let i = 0; i <= 4; i++) {
        ...
        let svalues = new Array();
        for (let l = 0; l <= 3; l++) {
            let part = new Array();
            for (let x = 0; x < uPrime.length; x++) {
                part.push(uPrime[x]**l);
            }
            svalues.push(part);
        }
        let gamma = solve(tdata, svalues, (useSVD = true))
        //console.log(gamma);
        //NOTE const timePoly = generateTimePoly;
        //  Find max deviation of points to fitted curve
        const max = findMaxError(points, first, last, curve, uPrime);
        if (max.error < error && parametersInOrder) {
            addCurve(segments, curve);
            gammasOutput=[...Gammas,gamma];
            return gammasOutput;
        }
        split = max.index;
    ...
    fitCubic(points, segments, error, first, split, tan1, tanCenter, Gammas);
    fitCubic(points, segments, error, split, last, tanCenter._negate(), tan2, Gammas);
};
...
const generateBezier = (points, first, last, uPrime, tan1, tan2) => {
    ...
    // Create the C (coefficient) and X matrices
    C = [
        [0, 0, 0],
        [0, 0, 0],
        [0, 0, 0]
...
//NOTE CREATED BY ME generates cubic polynomial to fit timestamps
/*const generateTimePoly = () => {
...
const simplifySvgPath = (points, tdata, options = {}) => {
    //[Gammas, setGammas] = useState([]);
    if (points.length === 0) {
        return '';
    }
    let a = fit(points.map(typeof points[0].x === 'number' ? (p) => new Point(p.x, p.y) : (p) => new Point(p[0], p[1])), tdata, options.closed, options.tolerance ?? 2.5);
    return [getSegmentsPathData(a[0], options.closed, options.precision ?? 5), a[1]];
};
