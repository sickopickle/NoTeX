  ...
  paths.map((i) => ({
    ...
    path: i.path
      ...
      : i.data.reduce(
          (acc: string[], data) => [
            ...acc,
            generateSVGPath(data, tdatas[acc.length], simplifyOptions),
          ],
          []
        ),
  }));
...
const Canvas = forwardRef<CanvasRef, CanvasProps>(
  (
    ...
    simplifyOptions = {
      ...
      amount: 25, // change to make less Bezier curves
      roundPoints: true,
      ...simplifyOptions,
    };
    const [timeDatas, setTimeDatas] = useState<number[][]>([]);
    const [paths, setPaths] = useState<PathType[]>([]);
    //NOTE if generateSVGPaths for initial paths is to work I have to modify the below line for timestamps for initial paths
    ...
    //NOTE if error occurs use an iterative loop instead of reduce
    /*const getTimestamps = () => {
      if (paths.length === 0) {
        return null;
      }
    ...
    //NOTE add t to { x, y }
      ...
      .onTouchesUp(() => {
        ...
        if (tool === DrawingTool.Brush) {
          setPaths((prev) => {
            //console.log("bruh");
            const newSVGPath = generateSVGPath(path, timeData, simplifyOptions);
            //console.log("2");
            if (prev.length === 0) {
              //console.log("3");
              setTimeDatas([...timeDatas, timeData]);
              //console.log("4");
              return [
                {
                  color,
            ...
            if (
              ...
              //var b = solve(svalues, timeData, (useSVD=true));
              //lastPath.tdata = [...lastPath.tdata, timeData];
